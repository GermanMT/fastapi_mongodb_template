...
async def update_student_data(student_id: str, student: StudentModel = Body(...)):
    ...
    try:
        ...
        return JSONResponse(
            ...
            content = JSONEncoder().encode({'message': f'Student with id {student_id} updated successfully'})
        )
    except HTTPException as error:
        return JSONResponse(status_code = error.status_code, content = JSONEncoder().encode({'message': error.detail}))
...
async def delete_student_data(student_id: str):
    try:
        ...
        return JSONResponse(
            status_code = status.HTTP_200_OK,
            content = JSONEncoder().encode({'message': f'Student with id {student_id} deleted successfully'})
        )
    except HTTPException as error:
        return JSONResponse(status_code = error.status_code, content = JSONEncoder().encode({'message': error.detail}))